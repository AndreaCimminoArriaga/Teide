...
public class TeideMain {
	...
	public static void main(String[] args) throws Exception {
		...
		parametersEvaluator.setLinksFile(parameters.getOutputMainRuleLinksFile());
		evaluator = new LinksEvaluator(parametersEvaluator);
		FrameworkConfiguration.traceLog.addLogLine(TeideMain.class.getCanonicalName(), "Evaluating links generated by the main rule");
		startTime = System.nanoTime();
		matrix = evaluator.evaluateEffectiveness();
		stopTime = System.nanoTime();
	    elapsedTime = (stopTime - startTime)/1000000;
	    FrameworkConfiguration.traceLog.addLogLine(TeideMain.class.getCanonicalName(), "Done in "+elapsedTime+" (ms)");
		
		// store results
		results.add(matrix.getCSVHeader());
		results.add(matrix.toCSVLine());
		// Evaluate support links		
		parametersEvaluator = LinksEvaluatorParametersReader.parseFromJSON(parametersFile);
		parametersEvaluator.setLinksFile(parameters.getFilteredLinksFile());
		evaluator = new LinksEvaluator(parametersEvaluator);
		FrameworkConfiguration.traceLog.addLogLine(TeideMain.class.getCanonicalName(), "Evaluating links filtered by the supporting rules");
		startTime = System.nanoTime();
		matrix = evaluator.evaluateEffectiveness();
		stopTime = System.nanoTime();
	    elapsedTime = (stopTime - startTime)/1000000;
		...
		FileUtils.writeLines(new File(parametersEvaluator.getResultsFile()), results);
