...
We present Teide, an hybrid approach to improve the precision of linking resources from the Web of Data. Teide works with link rules that are generated by genetic programming algorithm. Teide receives a base link rule (main link rule) and apply it in order to obtain a collection of candidate links, then relying on a set of supporting that are bootstrapped to analyse the neighbours of the
resources involved in each candidate link (the neighbours are the resources that can be reached by means of their object properties). Teide analyses how similar they are in order decide which of the candidate links must be kept or discarded as false positives.
